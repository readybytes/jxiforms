...
<?php
	if(isset($disablePlugins)){
		...
		{
?>		
		<div class="jxif-disable dashboard-icon action-icon-view jxif-padding02 center" title="<?php echo JText::_("COM_JXIFORMS_DASHBOARD_DISABLED_ACTION_ICON_TOOLTIP");?>">
			...
			<div class="enable-plugin-button">
				<input type="button" value="<?php echo JText::_('COM_JXIFORMS_DASHBOARD_ENABLE_PLUGIN_BUTTON');?>" class="btn" onclick="window.location.href='<?php echo JUri::base().'index.php?option=com_jxiforms&view=dashboard&task=enableActionPlugin&plugin='.$plugin->element.'&pluginName='.$plugin->name; ?>'"); />
			</div>
		</div>
