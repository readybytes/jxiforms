 ...
 *
 */
class UglyformsHelperUtils extends UglyformsHelper
{
	...
	{		
		...
		return UglyformsHelperUtils::sendEmail($subject, $message, $attachments, $emails);
	}
	...
	public function sendEmail($subject, $message, $attachments=null, $emails = array())
	{
		...
		if( !$mailfrom  || !$fromname ) {
			throw new Exception(Rb_Text::_('COM_UGLYFORMS_EXCEPTION_UTILS_NO_EMAILFROM_AND_FROMNAME_EXISTS'));
		}
		
		$message = html_entity_decode($message, ENT_QUOTES);
		$mail 	 = UglyformsFactory::getMailer()->setSender( array($mailfrom, $fromname))
	...
	{
		// if not already set
		if(defined('UGLYFORMS_EXIT')==false){
			define('UGLYFORMS_EXIT',$msg);
			return true;
		}
