 ...
 * 
 */
class UglyformsHtmlFieldmanipulator
{
	...
	private static function _getFilterScript($id, $name, $fields, $value, $control_name)
	{
		...
		uglyforms.jQuery(document).ready(function(){
			...
			uglyforms.element.fieldmanipulator.insert(
						'<?php  echo Rb_HelperUtils::jsCompatibleId($control_name?$control_name:$id);?>',
						uglyforms.jQuery('#'+ '<?php  echo Rb_HelperUtils::jsCompatibleId($id);?>'),
	...
	static function _setupManipulatorScript()
	{
		...
		(function($){
			...
			uglyforms.element.fieldmanipulator = {
				_queue 	: new Array(),
				_elem_count : 0, 
				...
				insert : function(prefix, elem, fields){
					...
					queue = uglyforms.element.fieldmanipulator._queue;
					queue.push(data);
				...
				init : function(){
					...
					queue = uglyforms.element.fieldmanipulator._queue;
					show_queue = uglyforms.element.fieldmanipulator._show_queue ;
					
					// initiliaze all manipulators on screen
					for(var index=0 ; index < queue.length; index++){
						var data = queue[index];
						uglyforms.element.fieldmanipulator.manipulate(data.prefix, data.elem, data.fields);
					}
					
					uglyforms.element.fieldmanipulator._show_queue = new Array();
					
					$('.jxif-fieldmanipulator').change(uglyforms.element.fieldmanipulator.init);
				},
			...
			$(document).ready(function(){
				uglyforms.element.fieldmanipulator._elem_count = $('.jxif-fieldmanipulator').length;
			});
