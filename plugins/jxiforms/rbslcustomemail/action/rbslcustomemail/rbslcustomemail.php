 ...
 *
 */
class JXiFormsActionRbslcustomemail extends JXiformsAction
{
	...
	public function process($data, $attachments)
	{
		...
		$to_email = $actionParams->get('email_id', '');
		$to_email = JXiFormsHelperRewriter::rewrite($to_email, $data);

		//invalid email id
		if(!JMailHelper::isEmailAddress($to_email)){
		     return false;
		}
		...
		$reply_to = $actionParams->get('reply_to', '');
		if (!empty($reply_to)){
			$reply_to = (JMailHelper::isEmailAddress($reply_to)) ? $reply_to : JXiFormsHelperRewriter::rewrite($reply_to, $data);
