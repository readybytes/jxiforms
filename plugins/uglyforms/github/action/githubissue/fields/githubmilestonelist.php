 ...
 * @author bhavya
 */
class UglyformsFormFieldGithubmilestonelist extends JFormFieldList
{	
	...
	public function getOptions()
	{
		...
		if(empty($milestones)){
			return Rb_Text::_('COM_UGLYFORMS_ACTION_GITHUB_NO_MILESTONE_EXISTS');
		}		
		
		$options = array();
		foreach ($milestones as $milestone){
			$options[] = UglyformsHtml::_('select.option', $milestone['id'], $milestone['name']);
		}
	...
	{  
		...
		$githubApp 	= UglyformsAction::getInstance($actionId);
		...
		$result 	= UglyformsActionGithubHelper::requestAPI($url, "GET", $username, $password);
